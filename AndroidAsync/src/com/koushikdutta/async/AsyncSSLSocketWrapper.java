			...
			try {
				...
				TrustManager[] trustAllCerts = new TrustManager[] { new X509TrustManager() {
					...
					public void checkServerTrusted(java.security.cert.X509Certificate[] certs, String authType) {
						// vcube changes
						if(isLoggable())Log.d("MainActivity", "checkServerTrusted:: ");
						// end vcube changes
						for (X509Certificate cert : certs) {
							if (cert != null && cert.getCriticalExtensionOIDs() != null) {
								...
								while (it.hasNext()) {
									...
									if(isLoggable())Log.d("MainActivity", "ext: " + ext);
								}
								// end of vcube changes
			...
			if (!finishedHandshake && (engine.getHandshakeStatus() == HandshakeStatus.NOT_HANDSHAKING || engine.getHandshakeStatus() == HandshakeStatus.FINISHED)) {
				if (clientMode) {
					...
					if (trustManagers == null) {
						// vcube changes
						if(isLoggable())Log.d("MainActivity",
					...
					for (TrustManager tm : trustManagers) {
						// vcube changes
						if(isLoggable())Log.d("MainActivity", "2222 handleHandshakeStatus:: tm: " + tm);
						// end of vcube changes
						...
						try {
							...
							if (mHost != null) {
								if (hostnameVerifier == null) {
									...
									verifier.verify(mHost, StrictHostnameVerifier.getCNs(peerCertificates[0]),
											StrictHostnameVerifier.getDNSSubjectAlts(peerCertificates[0]));
								} else {
							...
							// vcube changes
							if(isLoggable())Log.d("MainActivity", "4444 GeneralSecurityException", ex);
							// end of vcube changes
							peerUnverifiedCause = ex;
						}
						catch (SSLException ex) {
							// vcube changes
							if(isLoggable())Log.d("MainActivity", "4444 SSLException", ex);
							peerUnverifiedCause = ex;
							// end of vcube changes
						}
					}
					...
					if (!trusted) {
						// vcube changes
						if(isLoggable())Log.d("MainActivity", "5555, Certificate not trusted!!!! Throwing exception!!!");
						// end of vcube changes
